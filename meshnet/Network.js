...
class Network {
    ...
    async run(protocol, graph, T, typeSend, pSend, pMove, dMove = 1, dLink = 1, progress = null) {
        ...
        for (const t of range(T)) {
            ...
            for (const i of range(parseSendNumber(typeSend, pSend))) {
                const id = `${t}-${i}`;
                messages.push(id);
                const ss = randInt(this.users.length);
                const contacts = graph.get(ss);
                if (contacts.length === 0) {
                    console.warn('isolate user', ss, graph);
                    continue;
                }
                const tt = contacts[randInt(contacts.length)];
                const source = this.users[ss], target = this.users[tt];
                protocol.onSend(t, source, target, id);
            }
            // 1.5. do batching
